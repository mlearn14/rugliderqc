...
def main(args):
    ...
    for deployment in args.deployments:
        ...
        time_gaps.loc[1:, "tdiff"] = (
            np.diff(time_gaps["time"]).astype("timedelta64[m]").astype("float")
        )
        ...
        if (
            datetime.utcnow() - t[0]
        ).total_seconds() / 60 / 60 > ignore_gaps_younger_than:
            gap_criteria = (
                np.nanmax(
                    time_gaps["tdiff"][
                        time_gaps["time"]
                        < datetime.utcnow() - timedelta(hours=ignore_gaps_younger_than)
                    ]
                )
                > max_data_gap
            )
        ...
        if lag_criteria or gap_criteria:
            print(
                f"latest sci-profile data {max(t).strftime('%Y-%m-%d %H:%M')} ({'{0:.2f}'.format(latest_data_age)} hours)"
            )
            for i in np.where(time_gaps["tdiff"] > max_data_gap)[0]:
                tx = time_gaps["time"][i] - time_gaps["time"][i - 1]
                print(
                    f"data gap from {time_gaps['time'][i-1].strftime('%Y-%m-%d %H:%M')} to {time_gaps['time'][i].strftime('%Y-%m-%d %H:%M')} ({'{0:.2f}'.format(tx.total_seconds()/60/60)} hours)"
                )
        else:
