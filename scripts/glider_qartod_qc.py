...
def build_global_regional_config(ds, qc_config_root):
    ...
    profile_time = cf.convert_epoch_ts(ds["profile_id"])
    ...
    for region in region_bounds["regions"]:
        ...
        if not os.path.exists(
            os.path.join(qc_config_root, region["region"] + "_configs.yml")
        ):
            continue
        if Polygon(
            list(
                zip(region["boundaries"]["longitude"], region["boundaries"]["latitude"])
            )
        ).contains(Point(profile_lon, profile_lat)):
            best_region = region

    if best_region["region"] != "global":
        # Pull in regional config
        config_file = os.path.join(
            qc_config_root, best_region["region"] + "_configs.yml"
        )
...
def define_gross_flatline_config(instrument_name, model_name):
    ...
    if "instrument_ctd" in instrument_name:
        if np.logical_and(
            "sea-bird" in model_name.lower(), "gpctd" in model_name.lower()
        ):
        ...
        if np.logical_and(
            "aanderaa" in model_name.lower(), "4831" in model_name.lower()
        ):
            config_filename = "optode_4831_gross_flatline.yml"
        elif np.logical_and(
            "aanderaa" in model_name.lower(), "3835" in model_name.lower()
        ):
            config_filename = "optode_3835_gross_flatline.yml"
        elif np.logical_and(
            "aanderaa" in model_name.lower(), "3830" in model_name.lower()
        ):
            config_filename = "optode_3830_gross_flatline.yml"
        elif np.logical_and(
            "aanderaa" in model_name.lower(), "5014w" in model_name.lower()
        ):
            config_filename = "optode_5014W_gross_flatline.yml"
        elif np.logical_and(
            "aanderaa" in model_name.lower(), "5013w" in model_name.lower()
        ):
...
def main(args):
    ...
    if isinstance(deployments_root, str):
        ...
        for deployment in args.deployments:
            ...
            logging.info(
                "Running glider QARTOD QC: {:s}".format(
            ...
            for f in ncfiles:
                ...
                if not os.path.isdir(qc_config_root):
                    logging.warning(
                        "Invalid QC config root: {:s}".format(qc_config_root)
                    )
                    return 1

                # run gross and flat line tests
                # Set the path for the gross range and flat line configuration files
                qc_config_gross_flatline = os.path.join(
                    qc_config_root, "gross_flatline"
                )
                ...
                for inst in instruments:
                    ...
                    try:
                        ...
                        logging.error(
                            "Sensor maker and/or model not specified {:s}".format(inst)
                        )
                    ...
                    qc_config_file = os.path.join(
                        qc_config_gross_flatline, qc_config_filename
                    )

                    if not os.path.isfile(qc_config_file):
                        logging.debug(
                            "Missing QC configuration file: {:s} {:s}".format(
                                inst, qc_config_file
                            )
                        )
                        continue
                    ...
                    for cl in collected_list:
                        ...
                        attrs = cf.set_qartod_attrs(
                            test, sensor, c.config[sensor]["qartod"][test]
                        )
                ...
                try:
                    depth_rating = (
                        float("".join(filter(str.isdigit, ds.platform.depth_rating)))
                        * 1.05
                    )
                except ValueError:
                    # if depth rating isn't specified in the file, depth rating is the max glider depth rating (1050m)
                    depth_rating = float(1050)
                pressure_rating = gsw.p_from_z(
                    -depth_rating, np.nanmean(ds.profile_lat.values)
                )
                # cinfo = {'fail_span': [0, pressure_rating]}
                cinfo = {
                    "suspect_span": [0, pressure_rating],
                    "fail_span": [0, pressure_rating * 2],
                }
                qc_varname = f"{sensor}_qartod_gross_range_test"
                flag_vals = qartod.gross_range_test(inp=ds[sensor].values, **cinfo)

                # Define QC variable attributes, add a comment about the conversion from depth_rating in meters to dbar
                cinfo = {
                    "suspect_span": [0, int(depth_rating)],
                    "fail_span": [0, int(depth_rating * 2)],
                }
                attrs = cf.set_qartod_attrs(test, sensor, cinfo)
                attrs["comment"] = (
                    "Glider depth rating (m) in flag_configurations converted to pressure (dbar) from "
                ...
                logging.debug(
                    "Using configuration file for region: {:s} and time window: {:s} to {:s}".format(
                        c["region"],
                        c["window"]["starting"].strftime("%b-%d"),
                        c["window"]["ending"].strftime("%b-%d"),
                    )
                )
                ...
                for sensor, config_info in c["streams"].items():
                    ...
                    for test, cinfo in config_info["qartod"].items():
                        if test == "pressure_test":
                            ...
                            flag_vals[np.invert(non_nan_ind)] = (
                                qartod.QartodFlags.MISSING
                            )

                            # only run the test if the array has values
                            if len(non_nan_i) > 0:
                                flag_vals[non_nan_ind] = qartod.pressure_test(
                                    inp=data[non_nan_ind],
                            ...
                            climatology_settings = {
                                "tspan": [
                                    c["window"]["starting"] - dt.timedelta(days=2),
                            ...
                            if "depth_range" not in cinfo.keys():
                                climatology_settings["zspan"] = [
                                    0,
                                    np.nanmax(ds.depth.values),
                                ]

                                if "suspect_span" in cinfo.keys():
                                    climatology_settings["vspan"] = cinfo[
                                        "suspect_span"
                                    ]
                                ...
                                flag_vals = qartod.climatology_test(
                                    config=climatology_config,
                                    inp=data,
                                    tinp=times,
                                ...
                                if len(np.shape(cinfo["depth_range"])) == 1:
                                    ...
                                    if "fail_span" in cinfo.keys():
                                        climatology_settings["fspan"] = cinfo[
                                            "fail_span"
                                        ]
                                    ...
                                    for z_int in range(len(cinfo["depth_range"])):
                                        ...
                                        z_ind = np.logical_and(
                                            ds.depth.values
                                            > cinfo["depth_range"][z_int][0],
                                            ds.depth.values
                                            <= cinfo["depth_range"][z_int][1],
                                        )

                                        flag_vals[z_ind] = qartod.climatology_test(
                                            config=climatology_config,
                                            inp=data[z_ind],
                                            tinp=times[z_ind],
                            ...
                            if "suspect_threshold" in cinfo.keys():
                                spike_settings["suspect_threshold"] = cinfo[
                                    "suspect_threshold"
                                ] * np.nanmedian(tdiff)
                            if "fail_threshold" in cinfo.keys():
                                spike_settings["fail_threshold"] = cinfo[
                                    "fail_threshold"
                                ] * np.nanmedian(tdiff)
                            ...
                            # only run the test if the array has values
                            if len(non_nan_i) > 0:
                                flag_vals[non_nan_ind] = qartod.rate_of_change_test(
                                    inp=data[non_nan_ind],
                                    tinp=times[non_nan_ind],
                                    **cinfo,
                                )
                ...
                if len(qartod_vars) > 0:
                    processing_level_text = (
                        "Raw Slocum glider time-series dataset from the native data file format. "
                    ...
                    if mode == "delayed":
                        processing_level_text = (
                            f"{processing_level_text} Delayed mode dataset."
                        )
